...
def test_bootstrap_metric():
    ...
    npt.assert_allclose(util.sort_cols(expected_boot_rsquared),
                        util.sort_cols(plspm_calc.bootstrap().r_squared().drop(columns=columns_to_drop)), rtol=0.5)
    ...
    npt.assert_allclose(util.sort_cols(expected_boot_total_effects),
                        util.sort_cols(plspm_calc.bootstrap().total_effects().drop(columns=columns_to_drop)), rtol=0.6)
    ...
    npt.assert_allclose(util.sort_cols(expected_boot_paths),
                        util.sort_cols(plspm_calc.bootstrap().paths().drop(columns=columns_to_drop)), rtol=5)
    ...
    npt.assert_allclose(util.sort_cols(expected_boot_loadings),
                        util.sort_cols(plspm_calc.bootstrap().loading().drop(columns=columns_to_drop)), rtol=0.5)
